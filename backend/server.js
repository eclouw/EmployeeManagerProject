...
app.post('/api/employee/edit/submit', async (req, res)=>{
    ...
    const {first_name, last_name, email, emp_number, emp_role} = req.body;
    ...
    try{
        const query = "UPDATE public.employees SET first_name = $1, last_name = $2, email = $3, emp_role = $5 WHERE emp_number = $4";
        const queryResult = await pgData.query(query, [first_name, last_name, email, emp_number, emp_role]);
